...
- **Vague-verb matches action verbs only, not their noun forms.** A corpus replay over 100 real issues found the old `improv\w*|optimiz\w*` firing on terms of art (`self-improvement`, `optimizer`, `evaluator-optimizer`), which blocked legitimate creates. The regex now matches only `improve`/`enhance`/`optimize` verb and gerund forms. (This was the one validator-defect false-fail the replay surfaced; none of the Increment-1 changes introduced any.)
