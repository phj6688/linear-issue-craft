...
- **Headless / pipeline mode** (default when this skill is invoked from inside a subagent, or when filing more than one issue in a batch to hand straight to the execution pipeline): there is no human reading each draft, so the per-issue draft-approval ceremony and the per-issue Gate 3 reviewer are **dropped**. What replaces them is enforcement that cannot be skipped under load: **Gate 1 must pass on every block before every `save_issue` call** (a save-hook enforces this at the tool boundary; see the repo's hook), and the batch as a whole may get one review pass, not one reviewer per issue. The failure this fixes is real: the headless path is where per-issue ceremony silently evaporated (one run filed 80 issues with the validator never executed).
