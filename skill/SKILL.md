...
description: Use when creating, drafting, or rewriting Linear issues (epics, stories, hardening tickets, refactor tasks) in any workspace. Applies the linear-issue-craft conventions for titles, description structure, tone, labels, and epic-to-story decomposition. Trigger when the user says "draft an issue", "open an epic", "file a ticket", "write a Linear issue", or pastes a rough idea/bug report and asks for it to be filed.
---
...
5. **No em-dashes in prose.** The em-dash (`—`) is **only** allowed as the separator in role-prefixed story titles (e.g., `Venue — Demand forecasting`). Use periods, commas, or rewrite the sentence in body prose.
6. **Labels are layered, not flat.** Always pick one domain (e.g., `api` / `web` / `mobile`) **plus** one or more capabilities (e.g., `ai`, `seo`, `compliance`, `tech-debt`, `ui-ux`). Never use ownership labels like "frontend" or "urgent".
...
4. Draft the issue. Show it to the user before posting — never file without explicit approval of the final text.
5. If the user approves, use the `mcp__linear-server__save_issue` tool to file it.
...
## Anti-checks before showing the draft

Run these mentally before printing the draft to the user:

- [ ] Title contains a concrete noun (file path, endpoint, component name) or a Role-prefix.
- [ ] First paragraph of the body diagnoses current state.
- [ ] No em-dash anywhere except a Role title.
- [ ] No vague verbs: "improve", "enhance", "optimize" unless paired with a target metric.
- [ ] If story: exactly one PR's worth of scope.
- [ ] If epic: 2–8 stories named in the "Stories under this epic" section.
- [ ] If hardening: each numbered issue has a `**Fix:**` line directly under it.
- [ ] No "Generated with Claude" / "Co-Authored-By" / AI tells anywhere.

If any check fails, fix inline before showing the draft.
